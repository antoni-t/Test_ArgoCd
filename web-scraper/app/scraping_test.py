...
class ScrapingTest:
    ...
    def get_url(self, path):
        ...
        """
        return os.getenv('API_BASE_URL', "https://smard.de/app") + "/" + str(path)
    ...
    def fetch_and_insert_X_entries(self, num_values):
        ...
        for region in self.regions:
            for power_type in self.power_types:
                ...
                sorted_data = sorted(
                    filtered_data, key=lambda x: x[0], reverse=True)
                print("SORTED")
                print(sorted_data)
                ...
                result = [{'timestamp': item[0], 'wattage': item[1]}
                          for item in first_X]
                if len(result) > 0:
...
tester = ScrapingTest()
result = tester.fetch_and_insert_X_entries(10)
print(result)